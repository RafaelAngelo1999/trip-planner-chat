...
describe('Label Component', () => {
  describe('Renderização Básica', () => {
    ...
    it('deve aceitar className personalizada', () => {
      render(
        <Label className="custom-class text-lg" data-testid="custom-label">
          Custom Label
        </Label>
      );
    ...
    it('deve combinar classes padrão com classes personalizadas', () => {
      render(
        <Label className="text-red-500" data-testid="combined-label">
          Combined Classes
        </Label>
      );
      const label = screen.getByTestId('combined-label');
      expect(label).toHaveClass('text-sm', 'font-medium', 'leading-none', 'text-red-500');
    });
  });
  ...
  describe('Diferentes Tipos de Conteúdo', () => {
    ...
    it('deve renderizar JSX como children', () => {
      ...
      expect(label).toContainHTML('<span>Label with</span> <strong>JSX</strong>');
    });
  ...
  describe('Estados Especiais', () => {
    it('deve aceitar estado disabled via CSS', () => {
      render(
        <Label className="opacity-50 cursor-not-allowed" data-testid="disabled-label">
          Disabled Label
        </Label>
      );
    ...
    it('deve aceitar estado de erro via classes', () => {
      render(
        <Label className="text-red-500" data-testid="error-label">
          Error Label
        </Label>
      );
    ...
    it('deve aceitar estado de sucesso via classes', () => {
      render(
        <Label className="text-green-500" data-testid="success-label">
          Success Label
        </Label>
      );
    ...
    it('deve ativar input quando clicado', async () => {
      ...
      render(
        <div>
          ...
          <input id="focus-input" data-testid="focus-input" />
        </div>
      );
      ...
      render(
        <fieldset>
          ...
          <div>
            ...
            <input id="input1" data-testid="input1" />
          </div>
          <div>
            <Label htmlFor="input2">Input 2</Label>
            <input id="input2" data-testid="input2" />
          </div>
        </fieldset>
      );
  ...
  describe('Casos Extremos', () => {
    it('deve lidar com texto muito longo', () => {
      const longText = 'Este é um texto muito longo para testar como o Label se comporta com conteúdo extenso que pode quebrar em múltiplas linhas';
    ...
    it('deve lidar com caracteres especiais', () => {
      ...
      expect(screen.getByTestId('special-label')).toHaveTextContent(specialText);
    });
